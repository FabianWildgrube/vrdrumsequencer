...
{
    /// <summary>
    /// ADAPTATION OF THE MRTK TELEPORT POINTER
    /// changes teleportation behaviour to mimic SteamVR Home interaction:
    ///     touch down on the touchpad doesn't do anything
    ///     click down activates the pointer
    ///     click up triggers the actual teleport
    ///     touch up doesn't do anything
    ///     
    /// IMPORTANT: the methods startTeleportFlow and endTeleportFlow must be wired to an InputAction via a InputHandler component
    ///             AND the touchPad Click input must be wired to trigger that InputAction via a Controller Input Action Mapping!
    /// 
    /// 
    /// Original MRTK documentation:
    ...
    [AddComponentMenu("Scripts/MRTK/SDK/PFSVRTeleportPointer")]
    public class PFSVRTeleportPointer : CurvePointer, IMixedRealityTeleportPointer, IMixedRealityTeleportHandler
    {

        private bool teleportFlowInAction = false; //flag to prevent processing of touchpad input events when the user hasn't actually clicked the touchpad

        //This should be called when a click down happens on the touch pad
        public void startTeleportFlow(BaseInputEventData inputEventData)
        {
            if (inputEventData is InputEventData && (inputEventData as InputEventData).Handedness == this.Handedness)
            {
                teleportFlowInAction = true;
            }
        }

        //this should be called when a click up happens on the touch pad
        public void endTeleportFlow(BaseInputEventData inputEventData)
        {
            if (inputEventData is InputEventData && (inputEventData as InputEventData).Handedness == this.Handedness)
            {
                if (teleportFlowInAction && TeleportRequestRaised) //ensure this component actually raised a teleport request
                {
        ...
        {
            //Debug.Log("PFSVR Teleport pointer Input Changed " + eventData.InputData);
            using (OnInputChangedPerfMarker.Auto())
            {
                // Don't process input if we've got an active teleport request in progress.
                if (!teleportFlowInAction || isTeleportRequestActive || CoreServices.TeleportSystem == null)
                {
                    return;
                }
                ...
                if (currentInputPosition.sqrMagnitude > InputThresholdSquared)
                {
                    ...
                    if (!TeleportRequestRaised)
                    {
                        ...
                        if (absoluteAngle < teleportActivationAngle)
                        {
                            ...
                            Debug.Log("Raising Teleport Request");
